...
savePlot <-function(results, settings, phases, fileName){
  ...
  if (settings@xDimension == "Size"){
    ...
    data <- ddply(merged, c("Size", "Query", "Tool", "ChangeSet", "RunIndex", "MetricName"),
                  summarize, MetricValue=sum(MetricValue))
    data <- ddply(data, c("Size", "Query", "Tool", "ChangeSet", "MetricName"),
                  summarize, MetricValue=median(MetricValue))
  }
  else if(settings@xDimension == "Iteration"){
    data <- ddply(merged, c("Query", "Tool", "ChangeSet", "RunIndex", "MetricName", "Iteration"),
                  summarize, MetricValue=sum(MetricValue))
    data <- ddply(data, c("Query", "Tool", "ChangeSet", "MetricName", "Iteration"),
                  summarize, MetricValue=median(MetricValue))
  }
